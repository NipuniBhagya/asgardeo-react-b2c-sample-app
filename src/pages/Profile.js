...
const Profile = () => {
    ...
    useEffect(() => {
        if (userInfo) {
            setFormValues({
                ...
                id: userInfo?.id,
                mfa:
                    userInfo?.[SCHEMA]?.preferredMFAOption != null &&
                    JSON.parse(userInfo?.[SCHEMA]?.preferredMFAOption)?.authenticationOption,
    ...
    const handleSecondFactorSubmit = async (event) => {
        ...
        let _formData = {
            ...
            [SCHEMA]: {
                ...userInfo?.[SCHEMA],
                preferredMFAOption: '{"authenticationOption":"' + formValues?.mfa + '"}',
            },
        };
        try {
                    ...
                    <header className='App-header-sub-section'>
                        <div className='avatar-large'>
                            <img
                                ...
                                className='link-logo-image circular-image'
                            />
                        </div>
                        <h1>{`${formValues?.givenName} ${formValues?.lastName}`} </h1>
                        <tr>
                            <label>
                                Username: <b>{formValues?.username}</b>
                            </label>
                            <br />
                            <label>
                                User ID: <b>{formValues?.id}</b>
                            </label>
                        </tr>
                    </header>
                                            ...
                                            <tr>
                                                <td colSpan={2} className='tr-align-center'>
                                                    <label htmlFor='hint' className='label-hint'>
                                                        Changing the password will result in the termination of the
                                                        current session. You will have to login with the newly changed
                                                        password.
                                                    </label>
                                                </td>
                                            </tr>
                                            <button className='btn-outline btn-margin-top' type='submit'>
                                                Change
                                            </button>
                                        </table>
                                    </div>
                                ...
                                <form onSubmit={ handleSecondFactorSubmit }>
                                    <div className='table-container'>
                                        <table className='one-column-table'>
                                            ...
                                            <br />
                                            <button className='btn-margin-top btn-outline' type='submit'>
                                                Save
                                            </button>
                                        </table>
                                    </div>
                            ...
                            <tr>
                                <td colSpan={2} className='tr-padding tr-align-center'>
                                    <div className='notification tr-align-center' id='successNotification'>
                                        <p className='p-description' id='notificationDescription'>
